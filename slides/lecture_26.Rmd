...
$$\text{Full model:} \hspace{0.5cm} \text{body mass} = \beta_0 + \beta_1 \text{flipper length} + \beta_2 \text{IsChinstrap} + \beta_3 \text{IsGentoo} + \varepsilon$$

$$\text{Reduced model:} \hspace{0.5cm} \text{body mass} = \beta_0 + \beta_1 \text{flipper length} + \varepsilon$$
...
.large[
$$SSE_{reduced} - SSE_{full} = 52854796 - 47666988 = 5187808$$
]
...
$SSE_{reduced} - SSE_{full} = 5187808$
...
.large[
Observed test statistic (original data): $F = 18.393$
]
...
.large[
**IF** $H_0$ is true, $F \sim F_{p_{full} - p_{reduced}, n - p_{full}}$
]

.pull-left[
```{r, echo=F, message=F, warning=F, fig.width=7, fig.height=5}
hist(f_stats, xlab = "F", main = "Permutation samples")
```
...
hist(f_samp, xlab = "F", main = "Samples from F distribution")
```
]
