...
4. Fill in answers to questions 1 -- 9. I recommend you save and render as you answer each question, to avoid losing any work and to catch any knitting issues early.
...
[https://forms.gle/3Bao9iMM9mUtq2Nq8](https://forms.gle/3Bao9iMM9mUtq2Nq8)

Make sure to select the correct class section!
...
Once you have loaded the `nycflights13` package, the data are contained in the `flights` dataset. 

* Remember that you can use `?` to get help about functions and datasets in R, so `?flights` will tell you more about the flights data. 
* To see the full data itself in R, run `View(flights)` in the console

### Some helpful resources

* The [course codebook](https://sta112-s26.github.io/resources/codebook.html) provides an overview of exploratory data analysis in R, with code and examples
...
It is also important to know whether there is any missing data in the dataset. Missing data occurs when one or more of the variables in the data is not recorded for a row. To check for missing data with the `flights` dataset, we can use the following code:

```{r, eval=F}
colSums(is.na(flights))
```

This will return the number of missing values for each column of the data. If there are 0 missing values for a column, then there is no missing data in that column.

:::{.question}
#### Question 2

Use the code to look for missing values in the `flights` dataset. Which columns have missing values?
:::
...
Create a bar chart showing the number of flights departing from each of the three New York airports (EWR, LGA, and JFK). Which airport had the most flights? *Hint: the code for making a bar chart with `ggplot` is very similar to the code for making a histogram in Question 3. Just change the variable being plotted, and replace `geom_histogram` with something else.*
:::
